...
class SettingsRepository(private val context: Context) {
    ...
    val settingsFlow: Flow<KeyboardSettings> = context.dataStore.data.map { prefs ->
        ...
        KeyboardSettings(
            ...
            useImageBackground = prefs[Keys.USE_IMAGE_BACKGROUND] ?: false,
            backgroundOverlayAlpha = prefs[Keys.BACKGROUND_OVERLAY_ALPHA] ?: 0.5f
        )
    }
    ...
    suspend fun setBackgroundImagePath(path: String?) {
        context.dataStore.edit { prefs ->
            if (path != null) prefs[Keys.BACKGROUND_IMAGE_PATH] = path
            else prefs.remove(Keys.BACKGROUND_IMAGE_PATH)
        }
    }

    suspend fun setUseImageBackground(enabled: Boolean) {
        context.dataStore.edit { it[Keys.USE_IMAGE_BACKGROUND] = enabled }
    }

    suspend fun setBackgroundOverlayAlpha(alpha: Float) {
        context.dataStore.edit { it[Keys.BACKGROUND_OVERLAY_ALPHA] = alpha }
    }
